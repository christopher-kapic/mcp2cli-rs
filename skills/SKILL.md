...
Options:
  ...
  --oauth-client-secret S OAuth client secret
  --oauth-scope SCOPE     OAuth scope(s) to request
  --cache-key KEY         Custom cache key
  --cache-ttl SECONDS     Cache TTL (default: 3600)
  --refresh               Bypass cache
  --list                  List available subcommands
  --search PATTERN        Search tools by name (implies --list)
...
Sessions:
  --session-start NAME    Start a named session
...
Bake mode:
  ...
  bake install NAME         Create wrapper script (default: ~/.local/bin)
...
mcp2cli --mcp https://mcp.example.com/sse --read-resource "file:///path/to/resource"

# List and get prompts
mcp2cli --mcp https://mcp.example.com/sse --list-prompts
mcp2cli --mcp https://mcp.example.com/sse --get-prompt my-prompt --prompt-arg key=value
```
...
```bash
# Create a baked tool (--include/--exclude/--methods only in bake configs)
...
Filter options (`--include`, `--exclude`, `--methods`) are available in `bake create` and `bake update` only.

### Caching

Specs and MCP tool lists are cached (1h default TTL). Local files are never cached.
...
Prefer `--jq` over scripts for JSON processing — it uses fewer tokens and avoids script overhead.
...
`--head N` slices JSON arrays to the first N elements.
...
1. **Discover** all available commands:
   ```bash
   mcp2cli --mcp https://target.example.com/sse --list
   ```

2. **Inspect** each command to understand parameters:
   ```bash
   mcp2cli --mcp https://target.example.com/sse <command> --help
   ```

3. **Test** key commands and probe for edge cases:
   ```bash
   mcp2cli --mcp https://target.example.com/sse <command> --param value
   ```
   Specifically test for:
   - Large responses: use `--head 3` to preview — do any fields produce oversized output?
   - Date/time fields: what format does the API expect?
   - Pagination: does the API return all results or require `--offset`/`--limit`?
   - Error messages: what happens with invalid parameters?
   - Binary vs text responses: do any endpoints return non-JSON?
...
5. **Install** the wrapper into the skill's scripts directory:
   ```bash
   mcp2cli bake install myapi --dir .claude/skills/myapi/scripts/
   ```
...
6. **Create a SKILL.md** in `.claude/skills/myapi/` that teaches another AI agent how to use this API. The SKILL.md must go beyond `--help` output — focus on knowledge that can only be learned through testing and reading documentation.
   ...
   **Anti-Patterns & Gotchas** — document every surprise found during testing:
   - Date syntax quirks
   - Fields that produce oversized output (use `--head` or `--jq` to exclude)
   - Parameter name inconsistencies across endpoints
   - Scope/filtering confusion
   - Binary export corruption risks

   **Output Processing** — recommend `--jq` over scripts for JSON filtering:
   ```bash
   # Extract specific fields
